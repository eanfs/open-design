...
- `apps/aurora-control-plane`: private Aurora commercial control-plane boundary for commercial identity, billing, entitlements, credits, and account/task state. It must not treat the local daemon's SQLite, `RUNTIME_DATA_DIR`, or agent-runtime state as commercial truth.
